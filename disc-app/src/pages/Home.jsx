...
const FilterSidebar = ({ filters, onFilterChange, availableMajors, availableYears }) => {
  ...
  return (
    <div className="col-sidebar">
      <div className="filter-card">
        ...
        <div className="filter-group">
          <button
            onClick={() => onFilterChange({ major: 'All Majors', year: 'All Years' })}
...
function Home() {
  ...
  // Get unique majors and years from users for filter options
  const availableMajors = useMemo(() => {
    const majors = [...new Set(users.map(user => user.major).filter(Boolean))];
    return majors.sort();
  }, [users]);

  const availableYears = useMemo(() => {
    const years = [...new Set(users.map(user => user.graduationYear ? String(user.graduationYear).trim() : null).filter(Boolean))];
    return years.sort();
  }, [users]);
  ...
  const filteredUsers = useMemo(() => {
    return users.filter(user => {
      if (filters.major !== 'All Majors') {
        if (String(user.major || '').trim() !== String(filters.major).trim()) {
          return false;
        }
      }

      if (filters.year !== 'All Years') {
        const userYear = user.graduationYear ? String(user.graduationYear).trim() : '';
        const filterYear = String(filters.year).trim();
        ...
        <div className="col-content">
          ...
          {filteredUsers.length === 0 ? (
            <div style={{
              ...
              <p style={{ fontSize: '1.2rem' }}>No users found matching your filters.</p>
              <button
                onClick={() => setFilters({ major: 'All Majors', year: 'All Years' })}
